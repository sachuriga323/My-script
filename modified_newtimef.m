...
%     % Modified again 04/29/2011 due to comment in bug 1032
    ...
    %
    % FreqBinWidth (in Hz) is the interval between frequencies extracted by the FFT
    % and is equal to the inverse of the PADDED window size (in seconds).
    %
    % (1) Normalise the amplitude spectrum in alltfX by dividing by the TOTAL
    %     number of sample points in the zero-padded window (g.winsize * g.padratio).
    ...
    % For the amplitude spectrum in AmpSpectrum, each corresponding sine wave has a 
...
%    P  = alltfX.*conj(alltfX); % power for wavelets
    ...
    %
    % Note: P is now a real array containing the Power Spectral Density of
    % the signal. Note that wavelet analysis is NOT identical to frequency 
    % analysis, however, when using morlett wavelets, there is a strong 
